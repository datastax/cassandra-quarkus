...
[source,properties]
----
quarkus.cassandra.username=john
quarkus.cassandra.password=s3cr3t
----

=== Connecting to a cloud DataStax Astra database

When connecting to Astra, required properties to configure are: `secure-connect-bundle`,
which should point to a valid path to an Astra secure connect bundle, as well as `username` and
...
[source,properties]
----
quarkus.cassandra.secure-connect-bundle=/path/to/secure-connect-bundle.zip
quarkus.cassandra.username=john
quarkus.cassandra.password=s3cr3t
----
