...
====
We are using the specialized mapped producers for conversions between `CompletionStage` to `Uni`
and `Publisher` to `Multi`. To make it work, you need to adapt the `FruitMapperProducer`:
[source,java]
----
public class FruitMapperProducer {
...
----
Please note the `withResultProducers` method usage.
The `MutinyMappedResultProducer` provides a `MutinyMappedReactiveResultSet` reactive type that can be used in the `@Dao`.
The `UniMappedResultSetProducer` provides a conversion from `CompletionStage` to `Uni`.
