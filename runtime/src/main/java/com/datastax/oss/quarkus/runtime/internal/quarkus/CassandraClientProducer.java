...
public class CassandraClientProducer {
   ...
   *
   * <p>Codec classes are collected by the extension's build steps as class names and instantiated
   * here rather than at build time.
   */
  private void configureTypeCodecs(QuarkusCqlSessionBuilder builder) {
    LOG.debug("codecs = {}, codec provider methods = {}", typeCodecClasses, typeCodecProviders);
    ...
    for (String clz : typeCodecClasses) {
      if (providedClasses.contains(clz)) {
        LOG.debug("Codec {} was already provided by a provider method", clz);
      } else {
  ...
  /**
   * A {@code @CassandraCodecProvider public static TypeCodec<?>[] someMethod()} handing out codec
   * instances.
   */
