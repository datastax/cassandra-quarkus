...
@ThreadSafe
public abstract class MicroProfileMetricsUpdater<MetricT> implements MetricUpdater<MetricT> {
  public static final String CASSANDRA_METRICS_PREFIX = "cassandra";
  protected final Set<MetricT> enabledMetrics;
  protected final MetricRegistry registry;

  protected MicroProfileMetricsUpdater(Set<MetricT> enabledMetrics, MetricRegistry registry) {
  ...
  public <T extends Metric> T getMetric(MetricT metric, String profileName) {
    return (T) registry.getMetrics().get(new MetricID(buildFullName(metric, profileName)));
  }
